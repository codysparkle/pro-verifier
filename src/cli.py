...
@click.command()
@click.option(
    "--profiles", "-p", help="JSON string of profile URLs or path to JSON file"
)
@click.option(
...
def verify_profiles(profiles: str, output_dir: str, format: str, user_id: str) -> None:
    ...
    try:
        ...
        if not profile_urls:
            click.echo(
                "❌ No valid profile URLs found. Please provide profiles in JSON format.",
                err=True,
            )
            sys.exit(1)
        ...
        if not profiles_data:
            click.echo(
                "❌ Failed to fetch any profile data. Please check the URLs.", err=True
            )
            sys.exit(1)
        ...
        if format in ["markdown", "both"]:
            markdown_path = os.path.join(
                output_dir, f"verification_report{user_suffix}_{timestamp}.md"
            )
            click.echo(f"📝 Generating Markdown report: {markdown_path}")
            report_generator.generate_markdown_report(
                verification_report, markdown_path
            )
            reports_generated.append(markdown_path)

        if format in ["pdf", "both"]:
            pdf_path = os.path.join(
                output_dir, f"verification_report{user_suffix}_{timestamp}.pdf"
            )
...
def _display_summary(report: any, report_paths: List[str]) -> None:
    ...
    if report.analysis_summary:
        ...
        summary_preview = (
            report.analysis_summary[:200] + "..."
            if len(report.analysis_summary) > 200
            else report.analysis_summary
        )
        click.echo(f"   {summary_preview}")

    click.echo("\n" + "=" * 60)
    click.echo(
        "✨ Verification complete! Check the generated reports for detailed analysis."
    )
