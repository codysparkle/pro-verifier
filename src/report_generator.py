...
class ReportGenerator:
    ...
    def generate_markdown_report(self, report: VerificationReport, output_path: Optional[str] = None) -> str:
...
"""
        ...
        for profile in report.profiles_analyzed:
            verified_icon = "✅" if profile.verified else "❌" if profile.verified is False else "❔"
...
"""
        ...
        if report.discrepancies:
            ...
            for disc in report.discrepancies:
                severity_emoji = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(disc.severity, "❔")
...
"""
    ...
    def generate_pdf_report(self, report: VerificationReport, output_path: str):
        ...
        # Title
        title = Paragraph("Social Profile Verification Report", self.styles['CustomTitle'])
        story.append(title)
        ...
        trust_data = [
            ['Metric', 'Score', 'Assessment'],
            ['Overall Trust', f"{report.trust_score.overall}/100", self._get_assessment(report.trust_score.overall)],
            ['Reputation', f"{report.trust_score.reputation}/100", self._get_assessment(report.trust_score.reputation)],
            ['Consistency', f"{report.trust_score.consistency}/100", self._get_assessment(report.trust_score.consistency)],
            ['Content Quality', f"{report.trust_score.content_quality}/100", self._get_assessment(report.trust_score.content_quality)]
        ]
        ...
        for profile in report.profiles_analyzed:
            verified_text = "Yes" if profile.verified else "No" if profile.verified is False else "Unknown"
            followers = f"{profile.followers:,}" if profile.followers else "N/A"
            name = profile.name or "N/A"
        ...
        profile_table = Table(profile_data, colWidths=[1*inch, 1.5*inch, 1.5*inch, 0.8*inch, 0.7*inch])
